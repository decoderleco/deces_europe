...
pjan85 <- es_pb_age_max_age85 %>% left_join(es_pjan)
pjan90 <- es_pjan %>% anti_join(pjan85)
...
deces_annuel_age85 <- es_pb_age_max_age85 %>% left_join(es_deces_annuel) %>% filter(!is.na(age))
deces_annuel_age90 <- es_deces_annuel %>% anti_join(deces_annuel_age85)
...
deces_week20 <- es_deces_week %>% filter(str_sub(time,1,4) == "2020") 
...
es_deces_complet <- pop_deces_pays_age_quinq %>% left_join(deces_tot_20tot,by=c("sex","geo","agequinq"))
...
es_deces_complet <- es_deces_complet %>% bind_rows(deces_tot_20_2)

#gestion de l'Allemagne pour laquelle il manque les donn?es sexu?es et des moins de 40 ans en 2020
deces_weekDE <- es_deces_week %>% filter(geo == "DE") 

deces_tot_20DE <- deces_tot_20 %>% filter(geo == "DE")

#recuperation de la repartition de deces des moins de 40 en 2019
deces_completDE <- es_deces_complet %>% filter(geo == "DE") %>% filter(time == "2019-01-01")%>% group_by(agequinq,geo,time) %>% summarise(dc20=sum(dc20),population=sum(population),deces=sum(deces),deces_theo_2020=sum(deces_theo_2020))
...
es_deces_complet <- es_deces_complet %>% rbind(deces_tot_20DE)
#Ajout de l'ALlemagne 2020 en colonne
deces_completDE <- es_deces_complet %>% filter(geo == "DE")
...
es_deces_complet <- es_deces_complet %>% filter(geo != "DE")
es_deces_complet <- es_deces_complet %>% rbind(deces_completDE)

#calcul des deces theoriques en population française

deces_complet20 <- es_deces_complet %>% filter(time == "2020-01-01") %>% 
...
es_deces_complet <- es_deces_complet %>%  left_join(pop_france20)
es_deces_complet <- es_deces_complet %>%  mutate(deces_france_theo_20 = case_when(
  population == 0 ~ 0,
  TRUE ~ deces/(population)*pop_france20))


#groupement des donnees pour trouver les deces annuels
es_deces_complet_annuel <- es_deces_complet %>% filter(!is.na(population)) %>% group_by(geo,time) %>% 
...
es_deces_complet_annuel <- es_deces_complet_annuel %>% filter(!is.na(dc20)) %>% filter(!is.na(deces_theo_2020))
es_deces_complet_annuel <- es_deces_complet_annuel %>% mutate (augmentation20 = (dc20-deces_theo_2020)/deces_theo_2020)
...
deces_weekpays <- es_deces_week %>% filter(sex == "T") %>%  group_by(geo,time,age) %>% 
  summarise(deces=sum(values))
deces_weekFrance <- es_deces_week %>% group_by(geo,time) %>% 
...
owid_deces_standard_pays_semaine <- test3 %>% group_by(geo,time) %>% 
...
owid_deces_standard_pays_semaine <- owid_deces_standard_pays_semaine %>% left_join(num_semaine)
...
eu_mesures <- eu_mesures %>% mutate(date_start=as.Date(date_start),date_end=as.Date(date_end))
eu_mesures <- eu_mesures %>% mutate(time_start = paste0(isoyear(date_start),"W",as.integer(isoweek(date_start)/10),isoweek(date_start)-as.integer(isoweek(date_start)/10)*10))
eu_mesures <- eu_mesures %>% mutate(time_end = paste0(isoyear(date_end),"W",as.integer(isoweek(date_end)/10),isoweek(date_end)-as.integer(isoweek(date_end)/10)*10))
eu_mesures <- eu_mesures %>% mutate(geo=case_when(Country == "Austria"~"AT",
                                           Country == "Bulgaria"~"BG",
                                           Country == "Croatia" ~"HR",
                                           Country == "Estonia"~"EE",
                                           Country == "Germany"~"DE",
...
lockdown <- eu_mesures %>% filter(Response_measure == "StayHomeOrder")
owid_test <- num_semaine %>% rename (time_end =time, semaine_fin =numerosemaine)
lockdown <- left_join(lockdown,owid_test)
owid_test <- num_semaine %>% rename (time_start =time, semaine_debut =numerosemaine)
lockdown <- left_join(lockdown,owid_test)
owid_test <- lockdown %>% select(geo,Response_measure,time_start) %>% rename(time=time_start) %>% mutate(Response_measure="StayHomeOrderStart")
test2 <- lockdown %>% select(geo,Response_measure,time_end) %>% rename(time=time_end) %>% mutate(Response_measure="StayHomeOrderEnd")
owid_test <- owid_test %>% rbind(test2)
...
owid_covid_data <- owid_covid_data %>% mutate(date=as.Date(date))
...
owid_test <- ungroup(owid_covid_week) %>% 
        select(geo,
               time,
               new_deaths,
               new_cases,
...
owid_deces_standard_pays_semaine <- left_join(owid_deces_standard_pays_semaine,
                                        owid_test)
...
owid_nom_pays <- owid_nom_pays %>% mutate(zone=case_when( geo == "AL"~ "Est",
                                                geo == "AM"~ "Est",
                                                geo == "BG"~ "Est",
                                                geo == "CY"~ "Est",
                                                geo == "EE"~ "Est",
